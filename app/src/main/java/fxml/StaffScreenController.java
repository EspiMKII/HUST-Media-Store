...
public class StaffScreenController {
    ...
    @FXML
    private Button staffRefresh;

    @FXML
    private Button cartRefresh;
    ...
    void onCartCheckoutPressed(ActionEvent event) {
        staffInterface.checkOut(currentCart);
        cartStatus.setText("Checkout successful");
        currentCart.checkOut(); // Clear the current cart
        cartItems.clear();
        updateStaffPerformance(staffInterface.returnSelf());
    }

    @FXML
    void onRefreshPressed(ActionEvent event) {
        updateStaffPerformance(staffInterface.returnSelf());
        updateStaffDetails(staffInterface.returnSelf());
        updateCartDetails();
    }
